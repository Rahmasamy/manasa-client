      ...
      <div className="flex gap-2 mt-4 overflow-x-auto pb-2 scrollbar-hide scroll-smooth snap-x snap-mandatory">
        <div className="flex gap-2 min-w-max">
          {Questionsfilters.map((filter, index) => (
            <Button
              key={filter + index}
              className="hover:text-[#2885AC] text-white bg-[transparent] hover:bg-white border border-white py-2 px-3 sm:py-3 sm:px-4 text-sm sm:text-base whitespace-nowrap snap-start"
            >
              {filter}
            </Button>
          ))}
